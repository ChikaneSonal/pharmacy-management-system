...
def create_database_table():
    try:
        ...
        cursor.execute("CREATE DATABASE IF NOT EXISTS pharma")

        # Switch to the pharma database
        cursor.execute("USE pharma")

        # Create the pharmaco table
        cursor.execute("""
...
def open_reciept():
    ...
    def update_price(event):
        selected_item = itm_txt.get()
        if selected_item in products:
            rate_txt.delete(0, tk.END)
            rate_txt.insert(tk.END, products[selected_item])
    products = {'otocol-ac': 50, 'dermiford': 99, 'clop-g': 120, 'moov': 60, 'ponds super gel': 55, 'clenora': 100, 'parabec': 17, 'paralite': 300, 'xyz': 250, 'abc': 23, 'Tear Drop': 200}
    

    def insert_de():
        #customer_name = c_name.get()
        #phone_no = c_phone.get()
        customer_name = cname_txt.get()
        phone_no = cphone_txt.get()

        
        #customer_name = c_name.get()
        #phone_no = c_phone.get()
        product_name = itm_txt.get()
        quantity = quantity_txt.get()
        #quantity = Quantity.get()
        product_price = rate_txt.get()
        print("Customer Name:", customer_name)
        print("Phone No:", phone_no)
        print("Product Name:", product_name)
        print("Quantity:", quantity)
        print("Product Price:", product_price)
        if(customer_name=="" or phone_no=="" or product_name=="" or quantity==0 or product_price==0): 
            messagebox.showinfo("Insert status","All Fields are required",parent=root)

        #if(customer_name=="" or phone_no=="" or product_name=="" or quantity=="" or product_price==""): 
         #   messagebox.showinfo("Insert status","All Fields are required")
        else:
            mydb = mysql.connector.connect(host="localhost", user="root", password="loop", database="pharma")
            mycursor = mydb.cursor()
            mycursor.execute("insert into customers values('"+ customer_name +"','"+ phone_no +"','"+ product_name +"','"+ quantity +"','"+ product_price +"')")
            #mycursor.execute("INSERT INTO pharmaco VALUES (%s, %s, %s, %s, %s)", (customer_name, phone_no, product_name, quantity, product_price))
            #mycursor.execute("INSERT INTO pharmaco (customer_name, phone_no, product_name, product_quantity, product_price) VALUES (%s, %s, %s, %s, %s)", (customer_name, phone_no, product_name, quantity, product_price))
            ...
            mycursor.execute("commit");
            messagebox.showinfo("Insert status","Inserted Succesfully",parent=root);
            mydb.close();
   ...
   # itm_txt.bind("<<ComboboxSelected>>", update_price)
    itm_txt = ttk.Combobox(F2, values=['otocol-ac', 'dermiford', 'clop-g', 'moov', 'ponds super gel', 'clenora','parabec','paralite','xyz','abc','Tear Drop'], font=('Georgia', 8), width=25, height=15)
    itm_txt.bind("<<ComboboxSelected>>", update_price)
    itm_txt.set(options[0])
    itm_txt.grid(row=0, column=1, padx=9, pady=20)
    ...
    cname_txt.configure(state='normal')
    cphone_txt.configure(state='normal')
    #quantity_txt.configure(state='normal')
    itm_txt.focus_set()
    rate_txt.focus_set()
    ...
    btn3=Button(F2,text='Insert',font='arial 15 bold',padx=5,pady=10,command=insert_de,bg='lime',width=15)
...
def grab_date_mfg():
    global cal_mfg, date_window_mfg
    c1.delete(0, END)
    c1.insert(0, cal_mfg.get_date())
    
    date_window_mfg.destroy()

def pick_date_exp(event=None):
    global cal_exp, date_window_exp
    date_window_exp = Toplevel()
    date_window_exp.grab_set()
    date_window_exp.title('choose expiry date')
    date_window_exp.geometry('250x220+590+370')
    cal_exp = Calendar(date_window_exp, selectmode="day" , date_pattern="y/mm/dd")
    cal_exp.place(x=0, y=0)
    submit_btn=Button(date_window_exp, text="submit", command=grab_date_exp)
    submit_btn.place(x=80, y=190)

def grab_date_exp():
    global cal_exp, date_window_exp
    d1.delete(0, END)
    d1.insert(0, cal_exp.get_date())
    
    date_window_exp.destroy()
    date_window_exp.destroy()
    print("Date window destroyed")
...
c1.insert(0,'yyyy/mm/dd')
c1.bind("<FocusIn>", pick_date_mfg)
date_button_mfg = Button(window, text="Pick Date", command=pick_date_mfg)
date_button_mfg.place(x=400, y=210)
d = Label(window ,text = "Expiry date:",font=('Georgia',12))
d.place(x=50,y=240)
d1 = Entry(window)
d1.place(x=230,y=240)
d1.insert(0,'yyyy/mm/dd')
d1.bind("<FocusIn>", pick_date_exp)
date_button_exp = Button(window, text="Pick Date", command=pick_date_exp)
date_button_exp.place(x=400, y=240)
